...
console.log(
  "GOOGLE_CLIENT_ID:",
  process.env.GOOGLE_CLIENT_ID ? "Definido" : "No definido"
);
console.log(
  "YOUTUBE_API_KEY:",
  process.env.YOUTUBE_API_KEY ? "Definido" : "No definido"
);
console.log(
  "MAPBOX_TOKEN:",
  process.env.MAPBOX_TOKEN ? "Definido" : "No definido"
);
...
const allowedOrigins = [
  "https://deploy-geotube-front.vercel.app", //  fronted en producción
...
if (!GOOGLE_CLIENT_ID) {
  console.error(
    "ERROR: GOOGLE_CLIENT_ID no está definido en las variables de entorno"
  );
  process.exit(1);
}
...
const executeQuery = (query, params = []) =>
  db.promise().execute(query, params);
...
app.post("/api/comentarios-proyecto", authenticateToken, async (req, res) => {
  try {
    ...
    if (comentario.length > 1000) {
      return res
        .status(400)
        .json({ error: "El comentario no puede exceder los 1000 caracteres" });
    }
...
app.get("/api/comentarios-proyecto/estadisticas", async (req, res) => {
  try {
    ...
    const porcentajes = {
      cinco_estrellas:
        total > 0 ? ((stats.cinco_estrellas / total) * 100).toFixed(1) : 0,
      cuatro_estrellas:
        total > 0 ? ((stats.cuatro_estrellas / total) * 100).toFixed(1) : 0,
      tres_estrellas:
        total > 0 ? ((stats.tres_estrellas / total) * 100).toFixed(1) : 0,
      dos_estrellas:
        total > 0 ? ((stats.dos_estrellas / total) * 100).toFixed(1) : 0,
      una_estrella:
        total > 0 ? ((stats.una_estrella / total) * 100).toFixed(1) : 0,
    };

    res.json({
      total_comentarios: stats.total_comentarios,
      promedio_calificacion: stats.promedio_calificacion
        ? parseFloat(stats.promedio_calificacion).toFixed(1)
        : 0,
      distribucion: {
...
app.post("/api/auth/google", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error en autenticacion con Google: " + error.message });
  }
});
...
app.post("/api/auth/change-password", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error interno del servidor al cambiar la contraseña" });
  }
});
...
const saveVideosToDatabase = async (videos) => {
  const insertPromises = videos.map(
    ({ videoId, location_name, latitude, longitude }) => {
      return executeQuery(
...
app.get("/api/search", async (req, res) => {
  ...
  try {
    ...
    if (!location) {
      return res
        .status(404)
        .json({ error: "No se pudo encontrar la ubicacion en Mexico." });
    }
    ...
    if (!youtubeVideos || youtubeVideos.length === 0) {
      return res
        .status(404)
        .json({
          error: "No se encontraron videos de YouTube para esta ubicacion.",
        });
    }
...
app.get("/api/searchByCoords", async (req, res) => {
  ...
  try {
    ...
    if (!youtubeVideos || youtubeVideos.length === 0) {
      return res
        .status(404)
        .json({ error: "No se encontraron videos en esta ubicacion." });
    }
...
app.listen(port, () => {
  ...
  console.log(
    `Ruta Comentarios Proyecto: GET/POST ${host}:${port}/api/comentarios-proyecto`
  );
  console.log(`Ruta Google Auth: POST ${host}:${port}/api/auth/google`);
  console.log(`Ruta Perfil: GET/PUT ${host}:${port}/api/auth/profile`);
  console.log(`Ruta Foto Perfil: PUT ${host}:${port}/api/auth/profile/photo`);
  console.log(
    `Ruta Cambiar Contraseña: POST ${host}:${port}/api/auth/change-password`
  );
  console.log(`JWT Configurado: ${JWT_SECRET ? "SI" : "NO"}`);
  console.log(
    `Google Client ID Configurado: ${GOOGLE_CLIENT_ID ? "SI" : "NO"}`
  );
});
